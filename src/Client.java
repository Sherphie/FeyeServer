...
public class Client{
	...
	public Client() {
		...
		txt_name = new JTextField("Sherphie");
		...
		northPanel.setBorder(new TitledBorder("Connect information"));

		rightScroll = new JScrollPane(textArea);
		rightScroll.setBorder(new TitledBorder("Message history"));
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					try {
						...
						throw new Exception("Unavailable port No.!");
					}
					String hostIp = txt_hostIp.getText().trim();
					String name = txt_name.getText().trim();
					if (name.equals("") || hostIp.equals("")) {
						throw new Exception("Input name and server IP pls!");
					}
	...
	public boolean connectServer(int port, String hostIp, String name) {
		...
		try {
			...
			textArea.append("Connect with port：" + port + "    and IP：" + hostIp
					+ "   were failed!" + "\r\n");
			...
			while (true) {
				try {
					...
					if (command.equals("CLOSE"))// 服务器已关闭命令
					{
						...
						closeCon();// 被动的关闭连接
						JOptionPane.showMessageDialog(frame, "Server buffering is full！", "Error",
								JOptionPane.ERROR_MESSAGE);
						return;// 结束线程
					} else {// 普通消息
